...
def create_bot(bot_name, lex_role_arn, account_id, zip_bucket):
    response_data = {}
...
def handler(event, context):
    ...
    zip_bucket = event["ResourcePropoerties"]["lexZipBucket"]
    lex_role_arn = event["ResourceProperties"]["lex_role_arn"]
    account_id = context.invoked_function_arn.split(":")[4]
    if event["RequestType"] == "Create":
        try:
            response_data = create_bot(bot_name, lex_role_arn, account_id, zip_bucket)
